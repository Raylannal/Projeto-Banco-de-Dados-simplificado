...
void listarDadosTabela(FILE *arquivo, char nomeTabela[]) {
    ...
    fgets(linhaAtual, 255, arquivo); // Pega a primeira linha para que ela não seja impressa
    fgets(linhaAtual, 255, arquivo); // Pega a segunda linha que contém os tipos para que ela não seja impressa
    ...
    while (fgets(linhaAtual, maxTamLinha, arquivo) != NULL)
    {
        ...
        switch (colunaTabela[escolha].tipoColuna)
        {
            ...
            case TIPO_DOUBLE:
                ...
                break;
            case TIPO_STRING:
                printf("%s ||||| %s\n", colunaTabela[escolha].valorColuna.valorString, valorPesquisado);
                ...
                else if (strcmp(colunaTabela[escolha].valorColuna.valorString, valorPesquisado) < 0)
                {
                    strcpy(listaDeComparacao[indexValorMenor].valorMenor, linhaCopiaAtual);
                    indexValorMenor++;
                    break;
